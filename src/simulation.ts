...
import HistoricalData from './models/historicaldata'; // MongoDB model for historical data
import { logger, transactionLogger } from "./utils/logger";
import { buyToken } from "./swapper/buyToken";
import { MIN_TOKEN_AMOUNT, TOKEN_DETAILS } from "./config/profitConfig";
import SplitTokenHolders from "./models/splittokenholders";
import { calculateMovingAverage, calculateTokenAmountForUSDC, checkExclusiveTokenHolder } from "./utils/utils";
import mongoose from 'mongoose'; // Import mongoose to manage the database connection
...
async function simulateTradesFromHistoricalData(primaryWallet) {
    ...
    const historicalData = await HistoricalData.find({}); // Fetching historical trade data directly
    ...
    for (const trade of historicalData) {
        ...
        if (Number(buyTokenVolume) > tokenMinimumTransferAmount) {
            const exclusiveHolder = await checkExclusiveTokenHolder(tokenAddress, walletAddress);
           
            
            if (exclusiveHolder) {
                transactionLogger.info(`Simulating Whale Detected 🐋 for wallet: ${walletAddress}`);
                
                // Fetch the current price and calculate the moving average
                const currentPrice = priceAtBuy; // Using historical price at buy
                const movingAverage = await calculateMovingAverage(tokenSymbol, 50); // 50-period MA (adjust period as needed)

                // Check if the current price is above the moving average
                if (currentPrice > movingAverage) {
                    transactionLogger.info(`Simulating Trade: Price is above the 50-period moving average. Preparing to trade.`);
                    
                    // Calculate the amount of SOL to use for buying tokens based on historical data
                    const amountOfTokenOut = await calculateTokenAmountForUSDC(tokenAddress, AmountspentforBuy); // Amount to use for buying tokens
                        
                    // Simulate buying the token
                    await buyToken(
                        primaryWallet, // Keypair object representing the wallet to use for the transaction
                    ...
                    transactionLogger.info(`Simulated Trade Executed ✅ for token: ${tokenSymbol}`);
                    
                    // Record trade time to simulate trade history
                    await recordTradeTime(walletAddress);
                } else {
                    transactionLogger.info(`Simulating Trade: Price is below moving average. No trade executed.`);
                }
                ...
                transactionLogger.info(`Simulating Transaction ignored: Not an exclusive holder for wallet: ${walletAddress}.`);
            }
        } else {
            transactionLogger.warn(`Simulating Token transferred below minimum threshold: ${buyTokenVolume}`);
        }
    }
}
